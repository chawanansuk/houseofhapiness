...
| `/api/migrate` | (เจ้าของ) ย้ายข้อมูลจากชีตเข้าฐานข้อมูล Postgres ครั้งแรก / ดูสถานะฐานข้อมูล |
| `/api/order` | รับออเดอร์รูมเซอร์วิสจาก services.html บันทึกลงฐานข้อมูล (ถ้าไม่ได้เปิดฐานข้อมูล ลงชีตแท็บ Orders) แล้วส่ง LINE แจ้งเจ้าของ · ยอดคำนวณใหม่ฝั่งเซิร์ฟเวอร์ · จำกัด 40 ครั้ง/ชม./IP เพราะแขกทั้งตึกใช้ไวไฟ IP เดียวกัน |
...
| `LINE_CHANNEL_TOKEN` | ไม่บังคับ | Channel access token (long-lived) ของ LINE OA — ใช้แจ้งเตือนออเดอร์รูมเซอร์วิสใหม่ ไม่ตั้ง = ไม่แจ้ง |
| `LINE_NOTIFY_TO` | ไม่บังคับ | User ID ของผู้รับแจ้งเตือน ขึ้นต้นด้วย `U` — ผู้รับต้องแอด OA เป็นเพื่อนไว้ก่อน |
